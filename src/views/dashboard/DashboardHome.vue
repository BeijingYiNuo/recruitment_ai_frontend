<template>
  <div class="dashboard-home">
    ...
    >
      <div class="ctx-item" @click="handleContextAction('edit')">
...
async function handleContextAction(action) {
  ...
  switch (action) {
    case 'edit': router.push(`/dashboard/appointment?edit=${iv.id}`); break
    case 'cancel':
      if (iv.status === 'cancelled') { ElMessage.info('该面试已取消'); return }
      try {
        await ElMessageBox.confirm(`确定要取消「${iv.candidate_name}」的面试安排吗？`, '取消面试确认', { confirmButtonText: '确定取消', cancelButtonText: '暂不', type: 'warning' })
        await interviewApi.updateReserveSession(iv.id, { status: 'cancelled' }); ElMessage.success('已取消面试安排'); fetchInterviewSchedules(); fetchStats()
      } catch (e) { if (e !== 'cancel') ElMessage.error('取消失败: ' + (e?.detail || e?.message || '网络连接异常')) }
      break
    case 'restore':
      try {
        await ElMessageBox.confirm(`确定要将「${iv.candidate_name}」恢复为已预约状态吗？`, '恢复预约确认', { confirmButtonText: '确定恢复', cancelButtonText: '暂不', type: 'info' })
        await interviewApi.updateReserveSession(iv.id, { status: 'scheduled' }); ElMessage.success('已恢复预约'); fetchInterviewSchedules(); fetchStats()
      } catch (e) { if (e !== 'cancel') ElMessage.error('恢复失败: ' + (e?.detail || e?.message || '网络连接异常')) }
      break
